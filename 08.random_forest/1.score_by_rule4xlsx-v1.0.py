...
def compute_scores(df, methods='vs2-vb-dvf-gn'):
    ...
    # =========================================================
    # 🔥 6. Removal（显著增强，特别针对长序列）
    # =========================================================
    v_max = df.reindex(columns=['viral_genes', 'vs2_hallmark', 'gn_hallmarks']).fillna(0).max(axis=1)
    host = df.get('host_genes', 0)
    ...
    # 🔥 关键增强：长序列额外 penalty
    df.loc[long_seq & (host > v_max), "rm_score"] -= 1
