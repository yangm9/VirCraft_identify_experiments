...
for TYPE in "${DATASETS[@]}"; do
    ...
    # 定义当前处理的文件前缀名
    # 假设你的原始文件命名规则是: bacteria_plasmid_train.phigaro.fasta
    INPUT_FASTA="bacteria_plasmid_${TYPE}.phigaro.fasta"
    WORK_DIR="bacteria_plasmid_${TYPE}_phageboost"
    ...
    ls *.fasta | grep -v "${INPUT_FASTA}" | parallel -j 128 "PhageBoost -f {} -o {}_pb"
    ...
    bedtools getfasta -fi ${INPUT_FASTA} -bed ${TYPE}.complement.bed -fo ../bacteria_plasmid_${TYPE}.phigaro.phageboost.fasta
