...
def preprocess_matrix():
    ...
    for ds in datasets:
        ...
        # -- New: Perform balanced sampling only on the training set ---
        if ds == 'train':
            print('Performing stratified downsampling for training set...')
            # Group by origin_gradient, find the group with the smallest sample size (typically 10–20k)
            min_size = cleaned_df.groupby('origin_gradient').size().min()
            print(f'Min group size detected: {min_size}')
            
            # Sample by gradient and merge
